...
# Try to locate the C library and its header, using pkg-config hints
find_path(GMP_INCLUDE_DIR gmp.h HINTS ${PKG_gmp_INCLUDEDIR})
find_library(GMP_LIB gmp HINTS ${PKG_gmp_LIBDIR})

# Only search for C++ interface if CXX language is enabled
if(CMAKE_CXX_COMPILER_LOADED OR CMAKE_CXX_COMPILER)
...
# Check whether gmp can be used (C interface)
if(GMP_INCLUDE_DIR AND GMP_LIB)
  file(WRITE "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeTmp/src_find_gmp.c"
    "#include <gmp.h>\nint main() { mpz_t x; mpz_init_set_ui(x, 42); mpz_clear(x); return 0; }\n")
  try_compile(GMP_C_WORKS ${CMAKE_BINARY_DIR}
    ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeTmp/src_find_gmp.c
    CMAKE_FLAGS
      -DINCLUDE_DIRECTORIES='${GMP_INCLUDE_DIR}'
      -DLINK_LIBRARIES='${GMP_LIB}'
    OUTPUT_VARIABLE OUTPUT)
  if(NOT GMP_C_WORKS)
    file(APPEND ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeError.log
      "Performing C SOURCE FILE Test GMP_C_WORKS failed with the following output:\n"
      "${OUTPUT}\n")
  endif()
endif()

# Remove these variables from cache inspector
mark_as_advanced(GMP_INCLUDE_DIR GMP_LIB GMPXX_INCLUDE_DIR GMPXX_LIB GMPXX_WORKS GMP_C_WORKS)
...
if(CMAKE_CXX_COMPILER_LOADED OR CMAKE_CXX_COMPILER)
  find_package_handle_standard_args(GMP
    DEFAULT_MSG
    GMPXX_LIB GMPXX_INCLUDE_DIR GMP_INCLUDE_DIR GMP_LIB GMPXX_WORKS GMP_C_WORKS
  )
else()
  find_package_handle_standard_args(GMP
    DEFAULT_MSG
    GMP_INCLUDE_DIR GMP_LIB GMP_C_WORKS
  )
endif()
...
if(GMP_FOUND)
  ...
  if((CMAKE_CXX_COMPILER_LOADED OR CMAKE_CXX_COMPILER) AND GMPXX_LIB AND GMPXX_INCLUDE_DIR)
